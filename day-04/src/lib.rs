pub fn section_as_bits(s: &str) -> u128 {
    ...
    let (mut left, mut right) = (left.parse::<u128>().unwrap(), right.parse::<u128>().unwrap());
    if left > right {
...
pub fn p1(lines: &Vec<String>) -> usize {
    ...
    for line in lines {
        ...
        if overlap == left_bits { // right fully contained left
            total += 1;
            println!("{left}\t{left_bits:#0128b}");
            println!("{right}\t{right_bits:#0128b}");
            println!("\t{overlap:#0128b}\n");
        } else if overlap == right_bits { // left fully contained right
            total += 1;
            println!("{left}\t{left_bits:#0128b}");
            println!("{right}\t{right_bits:#0128b}");
            println!("\t{overlap:#0128b}\n");
...
mod tests {
    ...
    fn example2() {
        let fd = std::fs::File::open("example.txt").unwrap();
        let mut lines: Vec<String> = std::io::BufReader::new(fd).lines().map(|l| l.unwrap()).collect();
        let r = p2(&mut lines);
        assert_eq!(r, 4);
    }
